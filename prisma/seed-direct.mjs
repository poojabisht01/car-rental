...
// deterministic IDs
const ADMIN_ID = "admin-driveeasy-001";
const JOHN_ID = "user-john-doe-001";
...
const userId = JOHN_ID;
const adminHash = bcrypt.hashSync('admin123', 10);
const userHash = bcrypt.hashSync('user123', 10);

const insertUser = db.prepare('INSERT INTO User (id, name, email, password, phone, role, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)');
const now = new Date().toISOString();

insertUser.run(adminId, 'Admin User', 'admin@carrental.com', adminHash, '+1-800-555-0001', 'admin', now);
insertUser.run(userId, 'John Doe', 'john@example.com', userHash, '+1-800-555-0100', 'user', now);
console.log('✓ Created users');

// Cars
const insertCar = db.prepare(`
  INSERT INTO Car (id, name, brand, model, year, type, transmission, seats, fuel, pricePerDay, image, images, description, features, available, rating, reviews, location, mileage, createdAt)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
const carsData = [
  {
    ...
    description: 'The Toyota Camry is a reliable and comfortable mid-size sedan perfect for city and highway driving. Featuring a smooth ride and excellent fuel economy.',
  ...
  },
  {
    ...
    description: 'Experience the pinnacle of German engineering with the BMW 5 Series. This executive sedan combines stunning performance with luxurious comfort.',
    features: JSON.stringify(['Heated Seats','Sunroof','Apple CarPlay','Lane Assist','Parking Sensors','Premium Sound System']),
    rating: 4.9, reviews: 85, location: 'Airport',
  },
  {
    ...
    description: 'Feel the thrill of American muscle with the iconic Ford Mustang. Powerful V8 engine and sporty design deliver an exhilarating driving experience.',
    features: JSON.stringify(['Performance Exhaust','Sport Mode','Launch Control','Bluetooth','Backup Camera','Apple CarPlay']),
    rating: 4.8, reviews: 64, location: 'Downtown',
  },
  {
    ...
    description: 'The Honda CR-V is the perfect family SUV — spacious, practical, and fuel-efficient. Handles city streets and country roads with ease.',
    features: JSON.stringify(['All-Wheel Drive','Roof Rails','Spacious Boot','Air Conditioning','GPS Navigation','Backup Camera']),
    rating: 4.6, reviews: 143, location: 'City Center',
  },
  {
    ...
    description: 'Drive into the future with the Tesla Model 3. Zero emissions, lightning-fast acceleration, and a massive touchscreen make every drive extraordinary.',
    features: JSON.stringify(['Autopilot','Over-the-Air Updates','Supercharger Access','Glass Roof','Premium Sound','Mobile App Control']),
    rating: 4.9, reviews: 211, location: 'North Hub',
  },
  {
    ...
    description: 'The Mercedes E-Class represents the finest in German luxury motoring. Impeccable build quality and supreme comfort define this executive saloon.',
    features: JSON.stringify(['Burmester Sound','Massage Seats','Night Vision','Head-Up Display','Air Suspension','Ambient Lighting']),
    rating: 4.9, reviews: 56, location: 'Airport',
  },
  {
    ...
    rating: 4.4, reviews: 89, location: 'South Terminal',
  },
  {
    ...
    rating: 4.7, reviews: 77, location: 'North Hub',
  },
  {
    ...
    description: 'The Audi A6 combines progressive design with advanced quattro technology. A sophisticated executive saloon with a driver-focused cockpit.',
    ...
    rating: 4.8, reviews: 48, location: 'Downtown',
  },
  {
    ...
    description: 'The ultimate people-mover, seating 12 passengers in comfort. Perfect for group travel, airport transfers, or corporate shuttles.',
    ...
    rating: 4.5, reviews: 32, location: 'Airport',
  },
  {
    ...
    description: 'The Nissan Patrol is a full-size luxury SUV built for both comfort and serious off-road capability. Seating for 7 and a powerful engine.',
    features: JSON.stringify(['7 Seats','4x4 Off-Road','Air Suspension','360 Camera','Third Row Folding','Roof Rails']),
    rating: 4.6, reviews: 61, location: 'City Center',
  },
  {
    ...
    description: 'The Hyundai Elantra offers exceptional value with stylish design and advanced features. Great fuel efficiency and a comfortable interior.',
    ...
    rating: 4.3, reviews: 102, location: 'South Terminal',
  },
  {
    ...
    description: 'The Porsche 911 is the benchmark sports car. Sixty years of evolution produced a near-perfect machine delivering spine-tingling performance.',
    ...
    rating: 5.0, reviews: 29, location: 'Downtown',
  },
  {
    ...
    description: 'The Kia Sportage redefines value in the compact SUV segment. Bold styling, feature-rich interior, and solid build quality make it a top family choice.',
    ...
    rating: 4.5, reviews: 95, location: 'North Hub',
  },
  {
    ...
    description: 'The Range Rover Sport is the definitive luxury performance SUV. Extraordinary capability combined with breathtaking refinement.',
    ...
    rating: 4.9, reviews: 41, location: 'Airport',
  },
];
...
for (const car of carsData) {
  ...
  carIds.push(id);
  insertCar.run(
    id, car.name, car.brand, car.model, car.year, car.type,
    car.transmission, car.seats, car.fuel, car.pricePerDay,
    car.image, car.images, car.description, car.features,
    1, car.rating, car.reviews, car.location, 'Unlimited', now
  );
}
...
const insertBooking = db.prepare(`
  INSERT INTO Booking (id, userId, carId, startDate, endDate, totalDays, totalPrice, status, pickupLoc, dropoffLoc, driverName, driverPhone, notes, createdAt)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
for (const b of bookings) {
  insertBooking.run(
    cuid(), userId, carIds[b.carIdx],
    new Date(b.start).toISOString(), new Date(b.end).toISOString(),
    b.days, b.price, b.status, b.pickup, b.dropoff,
    'John Doe', '+1-800-555-0100', b.notes, now
  );
}
