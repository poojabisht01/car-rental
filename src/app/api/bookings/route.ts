...
export async function POST(req: Request) {
  try {
    ...
    const car = db.prepare('SELECT * FROM Car WHERE id = ?').get(carId) as { pricePerDay: number; available: number } | undefined;
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, user.id, carId, start.toISOString(), end.toISOString(), totalDays, totalPrice, 'pending', pickupLoc, dropoffLoc, driverName, driverPhone, notes || null, now);
