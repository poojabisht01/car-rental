...
export async function GET(req: Request) {
  try {
    ...
    const cars = await queryAll('SELECT * FROM Car ORDER BY createdAt DESC');
...
export async function POST(req: Request) {
  try {
    ...
    await execute(`INSERT INTO Car (id, name, brand, model, year, type, transmission, seats, fuel, pricePerDay, image, images, description, features, available, rating, reviews, location, mileage, createdAt)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
